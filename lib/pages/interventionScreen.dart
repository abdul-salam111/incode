...
class InterventiScreen extends GetView<InterventionController> {
...
class _InterventionListState extends State<_InterventionList> {
  ...
  void _onScroll() {
    if (navbarController.interventiScrollController.position.pixels >=
        navbarController.interventiScrollController.position.maxScrollExtent -
            200) {
      controller.fetchAllInterventions();
    }
  }

    @override
  Widget build(BuildContext context) {
    print("hasConnectionError.value: ${controller.hasConnectionError.value}");
    print("interventions.isEmpty: ${controller.interventions.isEmpty}");
          return Obx(
        () {
          if (controller.interventions.isEmpty && controller.hasConnectionError.value) {
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: controller.isLoading.value
                        ? null
                        : () async {
                            await controller.fetchAllInterventions();
                            if (controller.hasConnectionError.value) {
                              Utils.anotherFlushbar(context, "Errore di connessione. Riprova.", Colors.red);
                            } else {
                              Utils.anotherFlushbar(context, "Interventi caricati con successo", Colors.green);
                            }
                          },
        ...
        return SmartRefresher(
          ...
          child: ListView.builder(
            ...
            itemCount: controller.interventions.length + 1,
            itemBuilder: (context, index) {
              return Obx(() {
                if (index >= controller.interventions.length) {
                            ...
                            Center(
                              child: ElevatedButton(
                                onPressed: controller.isLoading.value
                                    ? null
                                    : () async {
                                        final prevCount = controller.interventions.length;
                                        controller.fetchAllInterventions();
                                        if (controller.hasConnectionError.value) {
                                          Utils.anotherFlushbar(context, "Errore di connessione. Riprova.", Colors.red);
                                        } else if (controller.interventions.length > prevCount) {
                                          Utils.anotherFlushbar(context, "Interventi caricati con successo", Colors.green);
                                        }
                                      },
                                style: ElevatedButton.styleFrom(
                ...
                return _buildInterventionCard(
                    context, controller.interventions[index]);
              });
            },
          ),
        );
...
Widget _buildInterventionCard(
    BuildContext context, InterventionModel intervention) {
  final DateTime dateTime =
      DateTime.parse(intervention.datetimeIntervention.toString());
...
if (isNotEmpty(intervention. descriptionCompany))
          ...
          if (isNotEmpty(intervention.descriptionTypeObject ) && detailsPointMap.isEmpty)
             Divider(),
...
List<Widget> _buildDetailsWidgets(
    Map<String, dynamic> detailsMap, BuildContext context) {
